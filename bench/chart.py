...
def main() -> int:
    ...
    ax.bar([i - 0.2 for i in x], vllm, 0.4, label="vLLM", color=VLLM)
    ax.bar([i + 0.2 for i in x], nano, 0.4, label="nano-vllm", color=NANO)
    for i, (n, v) in enumerate(zip(nano, vllm)):
        if n and v:
            ax.text(i, max(n, v) * 1.03, f"{v / n:.1f}×", ha="center", fontsize=9, color="#475569")
